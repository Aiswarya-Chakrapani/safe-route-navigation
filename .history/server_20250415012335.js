...
app.post('/api/route', upload.single('crimeData'), async (req, res) => {
  ...
  console.log("Route request received:", req.body);
  ...
  try {
    ...
    console.log("Coordinates:", { sourceLat, sourceLon, destLat, destLon });
    ...
    if (req.file) {
      try {
        ...
        // Continue without crime data rather than failing
      }
    }
    ...
    try {
      ...
      // Calculate distance
      const distanceKm = calculateRouteDistance(G, routePath);
      
      // Create Google Maps URL
      const googleMapsUrl = createGoogleMapsUrl(G, routePath);
      ...
      console.error("Complex route calculation failed:", routeError);
      console.log("Using simple route calculation instead");
      route = calculateSimpleRoute(sourceLat, sourceLon, destLat, destLon);
    }
    
    // Calculate processing time
    const processingTime = (Date.now() - startTime) / 1000;
    console.log(`Route calculation completed in ${processingTime} seconds`);
    ...
    try {
      const { sourceLat, sourceLon, destLat, destLon } = req.body;
      const fallbackRoute = calculateSimpleRoute(sourceLat, sourceLon, destLat, destLon);
