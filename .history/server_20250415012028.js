...
app.post('/api/route', upload.single('crimeData'), async (req, res) => {
  ...
  try {
    ...
    // Process the rest of your route calculation...
    
    // Return a simple response if there are issues with the calculation
    if (someErrorCondition) {
      // If route calculation fails, at least return the coordinates
      return res.json({
        success: true,
        route: {
          waypoints: [
            { lat: parseFloat(sourceLat), lon: parseFloat(sourceLon) },
          ...
          googleMapsUrl: `https://www.google.com/maps/dir/${sourceLat},${sourceLon}/${destLat},${destLon}/`
        }
      });
    }
    ...
    res.status(500).json({ 
      ...
      details: err.message,
      // Include a simple direct route as fallback
      fallbackRoute: {
        waypoints: [
          { lat: parseFloat(req.body.sourceLat || 0), lon: parseFloat(req.body.sourceLon || 0) },
          { lat: parseFloat(req.body.destLat || 0), lon: parseFloat(req.body.destLon || 0) }
        ]
      }
    });
  }
